...
async function findMember(guild, username) {
  ...
  let member = guild.members.cache.find(search);
  if (!member) {
    await guild.members.fetch();
    member = guild.members.cache.find(search);
  }
  if (!member) {
...
app.post("/notify", async (req, res) => {
  ...
  try {
    ...
    if (!embed) return res.status(400).json({ error: `Unknown notify type: ${type}` });
    await member.send({ embeds: [embed] });
    return res.json({ success: true, sentTo: member.user.tag });
  } catch (err) {
    if (err.code === 50007) return res.status(400).json({ error: "User has DMs disabled" });
...
app.post("/test", async (req, res) => {
  const { secret, username, by } = req.body;
  if (secret !== API_SECRET) return res.status(401).json({ error: "Unauthorized" });
  ...
  try {
    ...
    if (!member) return res.status(404).json({ error: `User "${username}" not found in server` });
    const embed = new EmbedBuilder()
      .setTitle("🧪 Staff Test — Bot is Working!")
      .setColor(0x5865f2)
      .setDescription("This is a test message confirming the bot can reach you via DM.")
      .addFields(
        { name: "✅ Status", value: "Bot is online and connected", inline: true },
        { name: "👤 Issued By", value: by || "Management", inline: true }
      )
      .setFooter({ text: "Staff test initiated successfully." })
      .setTimestamp();
    await member.send({ embeds: [embed] });
    return res.json({ success: true, message: "Test DM sent!", sentTo: member.user.tag });
  } catch (err) {
    if (err.code === 50007) return res.status(400).json({ error: "User has DMs disabled" });
    return res.status(500).json({ error: err.message || "Failed to send test DM" });
  }
});
